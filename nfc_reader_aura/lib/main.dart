...
class _NFCReaderScreenState extends State<NFCReaderScreen> {
  ...
  Future<void> _checkNFCAvailability() async {
    if (kIsWeb) {
      setState(() {
        _statusText = "Mode Web: NFC disimulasikan";
  ...
  Future<void> _startNFCScan() async {
    ...
    if (kIsWeb) {
      setState(() {
        _isScanning = true;
        _statusText = "Simulasi scan NFC di Web...";
        _statusColor = Colors.orange;
        _progressValue = null;
      });
      ...
      String fakeId = "WEB-${DateTime.now().millisecondsSinceEpoch}";
      String time =
          DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());

      setState(() {
        _nfcIds.insert(0, "$time | ID: $fakeId");
        _statusText = "Simulasi NFC berhasil ✅";
    ...
    NfcManager.instance.startSession(
      onDiscovered: (NfcTag tag) async {
        try {
          var data = tag.data;
          List<int>? identifier = data['nfca']?['identifier'];
